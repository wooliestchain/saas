    ...
    <table style="height: 80%; background-color: #bdbeef" >
        ...
        <?php foreach ($elements_page as $element) : ?>
            <tr>
                ...
                <td><?= $element['token'] ?> Pas de Token valide</td>
                <td><a  style="text-decoration: none; color: #589be7" href="grant_token.php?id=<?= $element['id'] ?>&username=<?= $element['username'] ?>&nom_user=<?= $element['nom_user'] ?>&email=<?= $element['email'] ?>">Délivrer le token</a></td>
